...
async def _send_presence_snapshot(user: User, websocket: WebSocket, db: Session) -> None:
    """Send initial presence snapshot for the user and accepted friends.

    Payload format:
      {"type": "presence_snapshot", "online": [
          {"id": int, "display_name": str, "status": str, "avatar_url": str | None}, ...
      ]}
    """
    # Always include current user
    users: list[User] = [user]

    # Try to collect accepted friends; on any failure just send self to avoid crashing.
    try:
        links_stmt = select(FriendLink).where(
            FriendLink.status == FriendRequestStatus.accepted,
            or_(
                FriendLink.requester_id == user.id,
                FriendLink.addressee_id == user.id,
            ),
        )
        links = list(db.execute(links_stmt).scalars())
        friend_ids: set[int] = set()
        for link in links:
            try:
                # Determine the counterpart ID regardless of request direction
                other_id = link.addressee_id if getattr(link, "requester_id", None) == user.id else link.requester_id
                if isinstance(other_id, int) and other_id != user.id:
                    friend_ids.add(other_id)
            except Exception:
                continue
        if friend_ids:
            friends_stmt = select(User).where(User.id.in_(friend_ids))
            users.extend(list(db.execute(friends_stmt).scalars()))
    except Exception:
        # Fallback: ignore friends if anything goes wrong
        pass
    ...
    for u in users:
        try:
            display_name = (u.display_name or u.login) if getattr(u, "display_name", None) is not None else getattr(u, "login", "")
            status_value = getattr(getattr(u, "presence_status", None), "value", None)
            if status_value is None:
                # Fallback to string if enum missing
                status_value = str(getattr(u, "presence_status", "offline"))
            entries.append(
                {
                    "id": u.id,
    ...
    # Stable sort by display name (case-insensitive)
    try:
        entries.sort(key=lambda e: str(e.get("display_name", "")).lower())
    except Exception:
        pass

    if websocket.application_state == WebSocketState.CONNECTED:
        try:
            await websocket.send_json({"type": "presence_snapshot", "online": entries})
        except (RuntimeError, WebSocketDisconnect):
            # Socket closed in between; ignore
            return


async def _send_direct_snapshot(user_id: int, websocket: WebSocket, db: Session) -> None:
    """Send initial direct conversations snapshot for the user.

    Payload format:
      {"type": "direct_snapshot", "conversations": [ ... ]}
    """
    try:
        conversations = load_user_conversations(user_id, db)
        serialized: list[dict[str, object]] = [
            serialize_conversation(conv, current_user_id=user_id, db=db) for conv in conversations
        ]
    except Exception:
        serialized = []
    ...
    if websocket.application_state == WebSocketState.CONNECTED:
        try:
            await websocket.send_json({"type": "direct_snapshot", "conversations": serialized})
        except (RuntimeError, WebSocketDisconnect):
            return
